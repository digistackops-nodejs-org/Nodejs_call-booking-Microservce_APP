# User UI - Testing Suite Documentation

## Overview
Complete testing suite for the User UI frontend service including Unit, Integration, and API tests with health check endpoints.
...
For production deployment, the User UI needs a Node.js server to serve health check endpoints.
...
app.listen(PORT, () => {
  console.log(`User UI server running on port ${PORT}`);
});
```
